...
function lifelineAnchors(node) {
  ...
  node.anchors.push(
    new _core.Point(node.rect.x + node.rect.width, node.rect.y + 25, _core.Direction.Right),
  );
  node.anchors.push(
    new _core.Point(node.rect.x + node.rect.width / 2, node.rect.y + 50, _core.Direction.Bottom),
  );
  node.anchors.push(
