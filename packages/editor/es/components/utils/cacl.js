...
export function roundFun(value, n) {
  ...
  var s = Math.round(value * Math.pow(10, n)) / Math.pow(10, n); // let rs = s.indexOf('.');
  // if (rs < 0) {
  //   s += '.';
  // }
...
export function eraseOverlapIntervals(intervals) {
  ...
  var res = [];
  var flag = -Infinity; //记录前一区间的结束值，此处一开始需取负无穷，因为必须保证这里一开始是最小的
...
export function calcRepeatIndex(newArr) {
  ...
  newArr.forEach(function (newAr, index) {
    ...
    obj[newAr] = obj[newAr] || []; // push the current index into the array
...
export function rgbaStringToRgb(rgba, isFilter) {
  ...
  if (typeof rgba === "string") {
    ...
    return (arr || []).map(function (item) {
      ...
      return parseInt(item); //字符串，连续的多个0也会存在，不会被去掉
      // return item;
    });
  } else {
    return [];
