...
import CustomizedDynamicForm, {
  FieldData,
  FormFieldGroup,
} from '../../../common/CustomizedDynamicForm';
...
const NodeCanvasProps: React.FC<ICanvasProps> = React.forwardRef(
  (
    ...
    const { rotate, lineWidth, strokeStyle, text, id, name, fillStyle } = data?.node || {};
    ...
    const { dataMethod, dataDot, dataSourceId, dataSourceUrl, reqData, dataPointReq } =
      property || {};
    ...
    const addVedioSource = () => {
      onAddVedioSource &&
        onAddVedioSource(data.node, [], data.node.property.video?.selectedRowKeys);
    };
    // 获得选中的视频直播源
              ...
              <Panel header={`样式-${statusObj[key]}`} key={key}>
                <Form form={propertyForm} onValuesChange={handlePropertyValuesChange}>
                  ...
                  <Row>
                    ...
                    <Col span={4}>
                      <Form.Item name={`${key}.showBkColor`} valuePropName="checked">
                        <Checkbox />
                      </Form.Item>
                    </Col>
                    <Col span={10}>
                  ...
                  <Form.Item name="selectDataPoint">
                    ...
                    >
                      {(data.node.property?.dataPointSelectedRows || []).map((item, index) => {
                        return (
                          <Option value={item.scopeMin + '~' + item.scopeMax} key={index}>
                            {item.dataName}
                          </Option>
                        );
                      })}
