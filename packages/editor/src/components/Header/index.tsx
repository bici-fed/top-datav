import type { Topology } from '@top-datav/core';
...
const Header: React.FC<HeaderProps> = React.forwardRef((props: HeaderProps, ref) => {
  const { canvas, scaleVal, history, rootRef, isSave, setIsSave, onScaleCanvas } = props;
    ...
    <div className={styles.toolsHeader}>
      ...
      {headTools.map((item, index) =>
        item ? (
          ...
          >
            <a className={styles.toolItem} onClick={() => handleHeaderClick(item.key)}>
              <CustomIcon type={item.icon} style={{ width: 18, height: 18 }} />
              <span>{item.name}</span>
            </a>
          </Tooltip>
        ) : (
          <span className={styles.toolSplit} key={index}></span>
        ),
      )}
      <span className={styles.toolSplit}></span>
      <a
