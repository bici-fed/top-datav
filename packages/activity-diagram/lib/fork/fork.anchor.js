...
function forkHAnchors(node) {
  node.anchors.push(new _core.Point(node.rect.x, node.rect.y + node.rect.height / 2, _core.Direction.Left));
  node.anchors.push(new _core.Point(node.rect.ex, node.rect.y + node.rect.height / 2, _core.Direction.Right));
  var dis = 5;
