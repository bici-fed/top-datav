...
exports.biciMeasure = biciMeasure;

function biciMeasure(ctx, node) {
  ctx.beginPath();
  ctx.fillStyle = '#EBEBEB';
  ctx.strokeStyle = '#ebebeb'; // Set line width
  ...
  ctx.fill();
  ctx.stroke(); // radiusRect(ctx,node.rect.x, node.rect.y-2, node.rect.width, node.rect.height+4,4)
  ...
  var step = node.rect.height / scale;
  var dataMax = 100;
  ...
  if (value > dataMax) value = dataMax; // 绘制数据柱状图

  ctx.beginPath();
  ctx.fillStyle = '#1890ff';
  var vh = (node.rect.height * (value - dataMin)) / (dataMax - dataMin); // ctx.fillRect(node.rect.x+1.5, node.rect.y+node.rect.height-vh, node.rect.width-3, vh+2);

  if (node.property.value > dataMin) {
    radiusRect(
      ctx,
      node.rect.x + 3,
      node.rect.y + node.rect.height - vh,
      node.rect.width - 6,
      vh,
      (node.rect.width - 6) / 2,
    );
  ...
  ctx.fillStyle = '#999999'; //  画直线，也就是刻度
  ...
  if (node.property.markChecked) {
    for (var i = scale; i >= 0; i--) {
      ...
      var txt = dataMin + ((scale - i) * (dataMax - dataMin)) / scale + '';
      txt = getFixed(txt, 2);
      ctx.fillText(
        txt + '',
        node.rect.x - (txt + '').length * 6 - 20,
        node.rect.y + i * step + 4.5,
      );
      ctx.closePath();
      ctx.stroke();
    }
  ...
  if (node.property.chartUnitChecked) {
    ctx.fillText(
      getFixed(node.property.value, node.property.dataDot) + ' ' + node.property.chartUnit,
      node.rect.x - 8,
      node.rect.y + node.rect.height + 20,
    );
  } else {
    ctx.fillText(
      getFixed(node.property.value, node.property.dataDot),
      node.rect.x - 8,
      node.rect.y + node.rect.height + 20,
    );
  }
  ...
  ctx.closePath();
  ctx.stroke(); // 绘制两端的分段颜色

  var dataColors = node.property.dataColors;
  dataColors = dataColors.filter(function (item) {
    return item.checked == true;
  });
  dataColors.sort(function (a, b) {
    return a.top - b.top;
  });
  var beforeHeight = 0;
  dataColors.map(function (item) {
    var top = item.top;
    ...
    if (dataMax - dataMin != 0) {
      // 分母不能为0
      var h = node.rect.height * ((top - dataMin) / (dataMax - dataMin));
      ctx.fillRect(node.rect.x - 4, node.rect.y + (node.rect.height - h), 4, h - beforeHeight); // ctx.fillRect(node.rect.x+node.rect.width-4, node.rect.y+(node.rect.height-h), 4, h-beforeHeight);
...
var radiusRect = function radiusRect(ctx, left, top, width, height, r) {
  var pi = Math.PI;
  ctx.beginPath();
