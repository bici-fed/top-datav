export function simpleText(ctx, node) {
  ...
  ctx.closePath();
  ctx.moveTo(node.rect.x, node.rect.y + 40); //ctx.lineTo(node.rect.ex, node.rect.y + 40);
  ...
  var fillStyle = node.fillStyle; // if(fillStyle){
...
function hexToRgb(str) {
  var r = /^\#?[0-9a-fA-F]{6}$/;
  var r2 = /^(rgb|rgba)/g; //test方法检查在字符串中是否存在一个模式，如果存在则返回true，否则返回false
