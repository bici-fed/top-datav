...
export var images = {};
export var Node = /*#__PURE__*/function (_Pen) {
  _inherits(Node, _Pen);
  ...
  _createClass(Node, [{
    ...
    value: function draw(ctx) {
      ...
      drawNodeFns[this.name](ctx, this); // Draw text.
    ...
    value: function drawImg(ctx) {
      ...
      img.onload = function () {
        ...
        Store.set(_this2.generateStoreKey('LT:imageLoaded'), true);

        if (!_this2.gif && gif) {
          _this2.gif = true;
          Store.set(_this2.generateStoreKey('LT:addDiv'), _this2);
        }
      };
    }
  }, {
    ...
    value: function calcRectByParent(parent) {
      ...
      var x = parent.rect.x + parent.paddingLeftNum + abs(parentW, this.rectInParent.x) + abs(parentW, this.rectInParent.marginLeft);
      var y = parent.rect.y + parent.paddingTopNum + abs(parentH, this.rectInParent.y) + abs(parentW, this.rectInParent.marginTop);
        ...
        if (timeline >= _item.start && timeline < _item.end) {
          ...
          if (_item.linear) {
            if (_item.state.rect.x !== _item.initState.rect.x) {
              this.rect.x = _item.initState.rect.x + (_item.state.rect.x - _item.initState.rect.x) * rate;
              rectChanged = true;
            }

            if (_item.state.rect.y !== _item.initState.rect.y) {
              this.rect.y = _item.initState.rect.y + (_item.state.rect.y - _item.initState.rect.y) * rate;
              rectChanged = true;
            }

            if (_item.state.rect.width !== _item.initState.rect.width) {
              this.rect.width = _item.initState.rect.width + (_item.state.rect.width - _item.initState.rect.width) * rate;
              rectChanged = true;
            }

            if (_item.state.rect.height !== _item.initState.rect.height) {
              this.rect.height = _item.initState.rect.height + (_item.state.rect.height - _item.initState.rect.height) * rate;
              rectChanged = true;
            }
            ...
            if (_item.state.borderRadius !== _item.initState.borderRadius) {
              this.borderRadius = _item.initState.borderRadius + (_item.state.borderRadius - _item.initState.borderRadius) * rate;
            }

            if (_item.state.lineWidth !== _item.initState.lineWidth) {
              this.lineWidth = _item.initState.lineWidth + (_item.state.lineWidth - _item.initState.lineWidth) * rate;
            }

            if (_item.state.rotate !== _item.initState.rotate) {
              this.rotate = _item.initState.rotate + (_item.state.rotate - _item.initState.rotate) * rate;
              rectChanged = true;
            }

            if (_item.state.globalAlpha !== _item.initState.globalAlpha) {
              this.globalAlpha = _item.initState.globalAlpha + (_item.state.globalAlpha - _item.initState.globalAlpha) * rate;
            }
            ...
            if (_item.state.value !== _item.initState.value) {
              this.value = (_item.initState.value || 0) + ((_item.state.value || 0) - (_item.initState.value || 0)) * rate;
            }
          } else {
