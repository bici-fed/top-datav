...
export var ActiveLayer = /*#__PURE__*/function (_Layer) {
  ...
  function ActiveLayer() {
    ...
    Store.set(_this.generateStoreKey('LT:ActiveLayer'), _assertThisInitialized(_this));
    return _this;
  }
  ...
  _createClass(ActiveLayer, [{
    ...
    value: function calcControlPoints() {
      if (this.pens.length === 1 && this.pens[0] instanceof Node) {
        ...
        this.rotateCPs = [new Point(this.pens[0].rect.x + this.pens[0].rect.width / 2, this.pens[0].rect.y - 35), new Point(this.pens[0].rect.x + this.pens[0].rect.width / 2, this.pens[0].rect.y)];
      ...
      var _getBboxOfPoints = getBboxOfPoints(this.getPoints()),
    ...
    value: function render(ctx) {
      ...
      try {
        for (_iterator22.s(); !(_step22 = _iterator22.n()).done;) {
          ...
          if (_item2 instanceof Line) {
            var _tmp = new Line(_item2);

            _tmp.setTID(TID);

            if (_tmp.lineWidth < 3) {
              var bk = new Line(_item2);
              bk.setTID(TID);
            ...
            if (!this.data.locked && !_item2.locked) {
              drawLineFns[_item2.name].drawControlPointsFn(ctx, _item2);
            }
          }
        }
      } catch (err) {
