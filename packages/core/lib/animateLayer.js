...
var AnimateLayer = /*#__PURE__*/function (_Layer) {
  ...
  function AnimateLayer() {
    ...
    _storeUtils.Store.set(_this.generateStoreKey('LT:AnimateLayer'), _assertThisInitialized(_this));

    if (!_this.options.animateColor) {
      _this.options.animateColor = '#ff6600';
    }

    _this.subscribeUpdate = _storeUtils.Store.subscribe(_this.generateStoreKey('LT:updateLines'), function (lines) {
      _this.updateLines(lines);
    });
    _this.subscribePlay = _storeUtils.Store.subscribe(_this.generateStoreKey('LT:AnimatePlay'), function (params) {
      if (params.stop) {
        if (params.tag) {
  ...
  _createClass(AnimateLayer, [{
    ...
    value: function getAnimateLine(item) {
      ...
      if (!l.fromArrowColor) {
        l.fromArrowColor = l.strokeStyle || _storeUtils.Store.get(this.generateStoreKey('LT:color'));
      }

      if (!l.toArrowColor) {
        l.toArrowColor = l.strokeStyle || _storeUtils.Store.get(this.generateStoreKey('LT:color'));
      }
