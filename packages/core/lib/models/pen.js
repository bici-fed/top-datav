...
})(PenType || (exports.PenType = PenType = {}));
...
var Pen = /*#__PURE__*/ (function () {
  ...
  function Pen(json) {
    ...
    this.name = void 0;
    this.tags = void 0;
    this.rect = new _rect.Rect(0, 0, 0, 0);
    this.lineWidth = 1;
    this.rotate = 0;
    this.offsetRotate = 0;
    this.globalAlpha = 1;
    ...
    if (json) {
      this.TID = json.TID;
      this.id = json.id || (0, _uuid.s8)();
      this.name = json.name || '';
      this.value = json.value;
      this.tags = Object.assign([], json.tags);

      if (json.rect) {
        this.rect = new _rect.Rect(json.rect.x, json.rect.y, json.rect.width, json.rect.height);
      }
  ...
  (0, _createClass2.default)(Pen, [
    {
      ...
      value: function render(ctx) {
        ...
        ctx.strokeStyle =
          this.strokeStyle || _storeUtils.Store.get(this.generateStoreKey('LT:color'));
      ...
      value: function click() {
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
            ...
            if (item.type !== _event.EventType.Click) {
              continue;
            }

            this[this.eventFns[item.action]] &&
      ...
      value: function dblclick() {
        ...
        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done; ) {
            ...
            if (item.type !== _event.EventType.DblClick) {
              continue;
            }
    ...
    },
    {
      ...
      value: function doSocketMqtt(item, msg, client) {
        if (
          item.action < _event.EventAction.Function ||
          item.action === _event.EventAction.StopAnimate
        ) {
          this[this.eventFns[item.action]](
            msg.value || msg || item.value,
            msg.params || item.params,
            client,
          );
        } else if (item.action < _event.EventAction.SetProps) {
          ...
          if (item.params || item.params === undefined) {
            _storeUtils.Store.set(this.generateStoreKey('LT:render'), true);
          }
        }
      },
    },
