...
exports.calcTextRect = calcTextRect;
exports.fillText = fillText;
exports.getLines = getLines;
exports.getWords = getWords;
exports.getWrapLines = getWrapLines;
...
function getWrapLines(ctx, words, maxWidth, fontSize) {
  var lines = [];
...
function calcTextRect(ctx, pen) {
  var lines = getLines(ctx, pen);
  var width = 0;

  var _iterator = (0, _createForOfIteratorHelper2.default)(lines),
    _step;
...
function text(ctx, node) {
  ...
  if (node.font.color) {
    ...
    ctx.fillStyle = _storeUtils.Store.get(node.generateStoreKey('LT:fontColor'));
  }
...
function iconfont(ctx, node) {
  ...
  ctx.fillStyle =
    node.iconColor ||
    _storeUtils.Store.get(node.generateStoreKey('LT:iconColor')) ||
    node.font.color;
  ctx.beginPath();
