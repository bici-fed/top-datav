...
function messageTextRect(node) {
  node.textRect = new _rect.Rect(node.rect.x + node.paddingLeftNum, node.rect.y + node.paddingTopNum, node.rect.width - node.paddingLeftNum - node.paddingRightNum, node.rect.height * 3 / 4 - node.paddingTopNum - node.paddingBottomNum);
