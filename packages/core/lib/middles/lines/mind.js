...
function calcMindControlPoints(l) {
  ...
  if (l.from.direction === _direction.Direction.Left || l.from.direction === _direction.Direction.Right) {
    l.controlPoints = [from, new _point.Point(from.x, from.y + h / 3), new _point.Point(from.x, l.to.y)];
  } else {
    l.controlPoints = [from, new _point.Point(from.x + w / 3, from.y), new _point.Point(l.to.x, from.y)];
  }
...
function pointInMind(point, l) {
  ...
  if (!points) {
    ...
    for (var i = 0.01; i < 1; i += 0.01) {
      points.push((0, _curve.getBezierPoint)(i, l.controlPoints[0], l.controlPoints[1], l.controlPoints[2], l.to));
    }
