...
function getControlPt(pt, to) {
  ...
  if ((pt.direction === _direction.Direction.Up || pt.direction === _direction.Direction.Bottom) && Math.abs(pt.x - to.x) < 3) {
    if (to.y > pt.y) {
  ...
  if ((pt.direction === _direction.Direction.Left || pt.direction === _direction.Direction.Right) && Math.abs(pt.y - to.y) < 3) {
    if (to.x > pt.x) {
