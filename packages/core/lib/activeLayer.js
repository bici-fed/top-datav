  ...
  _createClass(ActiveLayer, [{
    ...
    value: function calcControlPoints() {
      if (this.pens.length === 1 && this.pens[0] instanceof _node.Node) {
        ...
        this.rotateCPs = [new _point.Point(this.pens[0].rect.x + this.pens[0].rect.width / 2, this.pens[0].rect.y - 35), new _point.Point(this.pens[0].rect.x + this.pens[0].rect.width / 2, this.pens[0].rect.y)];
      ...
      this.sizeCPs = [new _point.Point(x1, y1), new _point.Point(x2, y1), new _point.Point(x2, y2), new _point.Point(x1, y2)];
      this.rotateCPs = [new _point.Point(x1 + (x2 - x1) / 2, y1 - 35), new _point.Point(x1 + (x2 - x1) / 2, y1)];
    ...
    value: function saveNodeRects() {
      ...
      try {
        for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
          ...
          if (item.type) {
            this.nodeRects.push(new _rect.Rect(item.from.x, item.from.y, item.rect.width, item.rect.height));
          } else {
            this.nodeRects.push(new _rect.Rect(item.rect.x, item.rect.y, item.rect.width, item.rect.height));
          }
    ...
    value: function saveChildrenRects(node) {
      ...
      try {
        for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
          ...
          this.childrenRects[item.id] = new _rect.Rect(item.rect.x, item.rect.y, item.rect.width, item.rect.height);
    ...
    value: function render(ctx) {
      ...
      if (this.data.locked > _status.Lock.Readonly || this.options.activeColor === 'transparent') {
        return;
      }
