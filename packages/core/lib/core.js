...
var _hoverLayer = require("./hoverLayer");

var _activeLayer = require("./activeLayer");

var _animateLayer = require("./animateLayer");

var _divLayer = require("./divLayer");

var _rect2 = require("./models/rect");

var _uuid = require("./utils/uuid");

var _canvas = require("./utils/canvas");

var _rect3 = require("./utils/rect");
...
var Topology = /*#__PURE__*/function () {
  function Topology(parent, options) {
    ...
    this.onMouseMove = function (e) {
      if (_this.scheduledAnimationFrame || _this.data.locked === _status.Lock.NoEvent) {
        return;
      ...
      if (_this.mouseDown && (_this.data.locked || !_this.moveIn.type)) {
        ...
        switch (_this.options.translateKey) {
          case _options.KeyType.Any:
            b = true;
            break;

          case _options.KeyType.Ctrl:
            if (e.ctrlKey) {
              b = true;
            }
        ...
        if (!_this.options.disableTranslate && b && _this.data.locked < _status.Lock.NoMove) {
      ...
      _this.raf = requestAnimationFrame(function () {
        ...
        switch (_this.moveIn.type) {
          case MoveInType.None:
            _this.hoverLayer.dragRect = new _rect2.Rect(_this.mouseDown.x, _this.mouseDown.y, e.x - _this.mouseDown.x, e.y - _this.mouseDown.y);
            break;
          ...
          case MoveInType.AutoAnchor:
            ...
            _this.hoverLayer.lineTo(_this.getLineDock(new _point.Point(e.x, e.y), _status.AnchorMode.In), arrow);

            _this.needCache = true;
            break;

          case MoveInType.LineFrom:
            _this.hoverLayer.lineFrom(_this.getLineDock(new _point.Point(e.x, e.y), _status.AnchorMode.Out));
          ...
          case MoveInType.LineControlPoint:
            ...
            if (_middles.drawLineFns[_this.moveIn.hoverLine.name] && _middles.drawLineFns[_this.moveIn.hoverLine.name].dockControlPointFn) {
              _middles.drawLineFns[_this.moveIn.hoverLine.name].dockControlPointFn(_this.moveIn.hoverLine.controlPoints[_this.moveIn.lineControlPoint.id], _this.moveIn.hoverLine);
            }

            _this.needCache = true;

            _storeUtils.Store.set(_this.generateStoreKey('LT:updateLines'), [_this.moveIn.hoverLine]);
    ...
    this.onmousedown = function (e) {
      ...
      switch (_this.moveIn.type) {
        ...
        case MoveInType.LineMove:
          _this.hoverLayer.initLine = new _line.Line(_this.moveIn.hoverLine);
        ...
        case MoveInType.HoverAnchors:
          _this.hoverLayer.line = _this.addLine({
            ...
            from: new _point.Point(_this.moveIn.hoverNode.rotatedAnchors[_this.moveIn.hoverAnchorIndex].x, _this.moveIn.hoverNode.rotatedAnchors[_this.moveIn.hoverAnchorIndex].y, _this.moveIn.hoverNode.rotatedAnchors[_this.moveIn.hoverAnchorIndex].direction, _this.moveIn.hoverAnchorIndex, _this.moveIn.hoverNode.id),
            fromArrow: _this.data.fromArrow,
            to: new _point.Point(_this.moveIn.hoverNode.rotatedAnchors[_this.moveIn.hoverAnchorIndex].x, _this.moveIn.hoverNode.rotatedAnchors[_this.moveIn.hoverAnchorIndex].y),
        ...
        case MoveInType.AutoAnchor:
          _this.hoverLayer.line = _this.addLine({
            name: _this.data.lineName,
            from: new _point.Point(_this.moveIn.hoverNode.rect.center.x, _this.moveIn.hoverNode.rect.center.y, _models.Direction.None, 0, _this.moveIn.hoverNode.id),
            fromArrow: _this.data.fromArrow,
            to: new _point.Point(_this.moveIn.hoverNode.rect.center.x, _this.moveIn.hoverNode.rect.center.y),
            toArrow: _this.data.toArrow,
            strokeStyle: _this.options.color,
            lineWidth: _this.data.lineWidth
          });
    ...
    this.onmouseup = function () {
      ...
      if (_this.hoverLayer.dragRect) {
        ...
        if (_this.activeLayer.pens && _this.activeLayer.pens.length > 1) {
          _this.dispatch('multi', _this.activeLayer.pens);
        } else if (_this.activeLayer.pens && _this.activeLayer.pens[0] && _this.activeLayer.pens[0].type === _pen.PenType.Line) {
          _this.dispatch('line', _this.activeLayer.pens[0]);
        } else if (_this.activeLayer.pens && _this.activeLayer.pens[0] && _this.activeLayer.pens[0].type === _pen.PenType.Node) {
        ...
        switch (_this.moveIn.type) {
              ...
              if (_this.hoverLayer.line.to.id) {
                if (!_this.options.disableRepeatLine) {
                  ...
                  var lines = _this.data.pens.filter(function (pen) {
                    return pen.type === _pen.PenType.Line && pen.from.isSameAs(_this.hoverLayer.line.from) && pen.to.isSameAs(_this.hoverLayer.line.to);
                  });
          ...
          case MoveInType.LineControlPoint:
            _storeUtils.Store.set(_this.generateStoreKey('pts-') + _this.moveIn.hoverLine.id, null);
    ...
    this.ondblclick = function (e) {
      if (_this.moveIn.hoverNode) {
        ...
        var isHoverLineText = _this.moveIn.hoverNode.getTextRect().hit(new _point.Point(e.offsetX, e.offsetY));
        ...
        //     );
        var _isHoverLineText = _this.moveIn.hoverLine.getTextRect().hit(new _point.Point(e.offsetX, e.offsetY));
    ...
    _storeUtils.Store.set(this.generateStoreKey('topology-data'), this.data);

    if (!options) {
      options = {};
    }

    var font = Object.assign({}, _options.DefalutOptions.font, options.font);
    options.font = font;
    this.options = Object.assign({}, _options.DefalutOptions, options);

    _storeUtils.Store.set(this.generateStoreKey('LT:color'), this.options.color || '#222222');

    _storeUtils.Store.set(this.generateStoreKey('LT:fontColor'), this.options.font.color || '#222222');
  ...
  _createClass(Topology, [{
    ...
    value: function setupDom(parent) {
      ...
      var id = this.id;
      this.activeLayer = new _activeLayer.ActiveLayer(this.options, id);
      this.activeLayer.topology = this;
      this.hoverLayer = new _hoverLayer.HoverLayer(this.options, id);
      this.animateLayer = new _animateLayer.AnimateLayer(this.options, id);
      this.offscreen = new _offscreen.Offscreen(this.parentElem, this.options, id);
      this.canvas = new _renderLayer.RenderLayer(this.parentElem, this.options, id);
      this.divLayer = new _divLayer.DivLayer(this.parentElem, this.options, id);
    ...
    value: function setupSubscribe() {
      ...
      this.subcribe = _storeUtils.Store.subscribe(this.generateStoreKey('LT:render'), function () {
        _this2.render();
      });
      this.subcribeRender = _storeUtils.Store.subscribe('LT:render', function () {
        _this2.render();
      });
      this.subcribeImage = _storeUtils.Store.subscribe(this.generateStoreKey('LT:imageLoaded'), function () {
      ...
      this.subcribeAnimateMoved = _storeUtils.Store.subscribe(this.generateStoreKey('LT:rectChanged'), function (e) {
        _this2.activeLayer.updateLines(_this2.data.pens);
      });
      this.subcribeMediaEnd = _storeUtils.Store.subscribe(this.generateStoreKey('mediaEnd'), function (node) {
        if (node.nextPlay) {
      ...
      this.subcribeAnimateEnd = _storeUtils.Store.subscribe(this.generateStoreKey('animateEnd'), function (e) {
        if (!e) {
          return;
        }
    ...
    value: function setupMouseEvent() {
      ...
      if ((0, _utils.isMobile)()) {
        ...
        this.divLayer.canvas.ontouchstart = function (event) {
          ...
          var pos = new _point.Point(event.changedTouches[0].pageX - window.scrollX - (_this3.canvasPos.left || _this3.canvasPos.x), event.changedTouches[0].pageY - window.scrollY - (_this3.canvasPos.top || _this3.canvasPos.y));
        ...
        this.divLayer.canvas.ontouchmove = function (event) {
          ...
          if (len > 1) {
            ...
            if (len === 2) {
              ...
              var _pos = new _point.Point(event.changedTouches[0].pageX - window.scrollX - (_this3.canvasPos.left || _this3.canvasPos.x), event.changedTouches[0].pageY - window.scrollY - (_this3.canvasPos.top || _this3.canvasPos.y));
        ...
        this.divLayer.canvas.onmouseup = function (event) {
          ...
          var node = new _node.Node(_this3.touchedNode);
      ...
      this.divLayer.canvas.onwheel = function (event) {
        ...
        event.preventDefault();
        event.stopPropagation();
        var pos = new _point.Point(event.x - window.scrollX - (_this3.canvasPos.left || _this3.canvasPos.x), event.y - window.scrollY - (_this3.canvasPos.top || _this3.canvasPos.y));
      ...
      switch (this.options.keydown) {
        case _options.KeydownType.Document:
          document.addEventListener('keydown', this.onkeydown);
          break;

        case _options.KeydownType.Canvas:
    ...
    value: function ontouchend(event) {
      ...
      var node = new _node.Node(this.touchedNode);
    ...
    value: function dropNodes(jsonList, offsetX, offsetY) {
      ...
      jsonList.forEach(function (json) {
        ...
        if (json.type === _pen.PenType.Line) {
          _this4.addLine(Object.assign({
            name: 'line',
            from: new _point.Point(json.rect.x, json.rect.y),
            fromArrow: _this4.data.fromArrow,
            to: new _point.Point(json.rect.x + json.rect.width, json.rect.y + json.rect.height),
            toArrow: _this4.data.toArrow,
            strokeStyle: _this4.options.color
          }, json), true);
        } else {
          var node = new _node.Node(json);
    ...
    value: function addNode(node) {
      ...
      if (!_middles.drawNodeFns[node.name]) {
        return null;
      } // if it's not a Node


      if (!node.init) {
        node = new _node.Node(node);
      }
      ...
      if (node.autoRect) {
        ...
        var rect = (0, _middles.calcTextRect)(ctx, node);
    ...
    value: function open(data) {
      ...
      this.lock(data.locked || _status.Lock.None);
      ...
      _storeUtils.Store.set(this.generateStoreKey('LT:scale'), this.data.scale);
      ...
      if (data.nodes) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            item.TID = this.id;
            this.data.pens.push(new _node.Node(item));
          }
        } catch (err) {
          _iterator.e(err);
        } finally {
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            this.data.pens.push(new _line.Line(_item));
          }
        } catch (err) {
          _iterator2.e(err);
        } finally {
                ...
                if (hasChart) {
                  ...
                  try {
                    for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
                      ...
                      child.TID = this.id;
                      this.data.pens.push(new _node.Node(child));
                    }
                  } catch (err) {
                    _iterator5.e(err);
                  } finally {
                  ...
                  this.data.pens.push(new _node.Node(_item2));
                }
              } else {
                this.data.pens.push(new _node.Node(_item2));
              }
            } else {
              this.data.pens.push(new _line.Line(_item2));
            }
          }
        } catch (err) {
          _iterator3.e(err);
    ...
    value: function openSocket(url) {
      ...
      if (url || this.data.websocket) {
        this.socket = new _socket.Socket(url || this.data.websocket, this.data);
      }
    }
  }, {
    key: "closeSocket",
    ...
    value: function openMqtt(url, options) {
      ...
      if (url || this.data.mqttUrl) {
        this.mqtt = new _mqtt.MQTT(url || this.data.mqttUrl, options || this.data.mqttOptions, this.data.mqttTopics, this.data);
      }
    }
  }, {
    key: "closeMqtt",
    ...
    value: function getMoveIn(pt) {
      ...
      if (this.activeLayer.pens.length > 1 && (0, _canvas.pointInRect)(pt, this.activeLayer.sizeCPs)) {
      ...
      if (!this.data.locked) {
        ...
        try {
          for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
            ...
            if (item instanceof _line.Line && !item.locked) {
      ...
      for (var _i2 = len - 1; _i2 > -1; --_i2) {
        if (this.data.pens[_i2].type === _pen.PenType.Node && this.inNode(pt, this.data.pens[_i2])) {
          if (inLine && this.moveIn.type !== MoveInType.HoverAnchors) {
            this.inLine(pt, inLine);
          }

          return;
        } else if (this.data.pens[_i2].type === _pen.PenType.Line && this.inLine(pt, this.data.pens[_i2])) {
          // 优先判断是否在节点锚点上
    ...
    value: function inChildNode(pt, children) {
      ...
      for (var i = len - 1; i > -1; --i) {
        ...
        if (item.type === _pen.PenType.Line) {
          if (this.inLine(pt, item)) {
            return item;
          }
    ...
    value: function inNode(pt, node) {
      ...
      if (this.data.locked === _status.Lock.NoEvent || !node.visible || node.locked === _status.Lock.NoEvent) {
        return null;
      }
        ...
        if (!this.data.locked && !node.locked && !(this.options.hideAnchor || node.hideAnchor || node.rect.width < 20 || node.rect.height < 20)) {
          for (var j = 0; j < node.rotatedAnchors.length; ++j) {
            if (node.rotatedAnchors[j].hit(pt, this.options.anchorSize)) {
              if (!this.mouseDown && node.rotatedAnchors[j].mode === _status.AnchorMode.In) {
                continue;
              }
      ...
      if (node.hitInSelf(pt, this.options.anchorSize)) {
        for (var _j = 0; _j < node.rotatedAnchors.length; ++_j) {
          if (node.rotatedAnchors[_j].hit(pt, this.options.anchorSize)) {
            if (!this.mouseDown && node.rotatedAnchors[_j].mode === _status.AnchorMode.In) {
              continue;
            }
    ...
    value: function inLine(point, line) {
      if (this.data.locked === _status.Lock.NoEvent || !line.visible || line.locked === _status.Lock.NoEvent) {
        return null;
      }
    ...
    value: function getLineDock(point) {
      var mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _status.AnchorMode.Default;
    ...
    value: function getPensInRect(rect) {
      ...
      try {
        for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
          ...
          if (item.locked === _status.Lock.NoEvent) {
            continue;
          }
        ...
        for (_iterator10.s(); !(_step10 = _iterator10.n()).done;) {
          ...
          try {
            for (_iterator11.s(); !(_step11 = _iterator11.n()).done;) {
              ...
              if (!(item instanceof _node.Node) || this.activeLayer.has(item) || item.name === 'text') {
                continue;
              }
    ...
    value: function cache() {
      ...
      var data = new _data.TopologyData(this.data);
      ...
      if (pens && pens.length) {
        ...
        for (var i = 0, len = pens.length; i < len; i++) {
          ...
          if (pen instanceof _node.Node) {
            needPenMap[id] = new _node.Node(pen);
          } else if (pen instanceof _line.Line) {
            needPenMap[id] = new _line.Line(pen);
          }
        }
    ...
    value: function undo() {
      ...
      var data = new _data.TopologyData(this.caches.list[--this.caches.index]);
    ...
    value: function toImage() {
      ...
      var rect = this.getRect();
      var p = (0, _padding.formatPadding)(padding || 0);
      rect.x -= p[3];
      rect.y -= p[0];
      rect.width += p[3] + p[1];
      ...
      try {
        for (_iterator13.s(); !(_step13 = _iterator13.n()).done;) {
          ...
          if (item.type) {
            pen = new _line.Line(item);
          } else {
            pen = new _node.Node(item);
          }
    ...
    value: function _delete(param, force) {
      ...
      if (typeof param === 'string') {
        deleted = (0, _canvas.del)(param, this.data.pens);
      } else {
        ...
        for (var i = 0; i < pens.length; i++) {
          ...
          if ((0, _canvas.del)(item.id, this.data.pens).length) {
            deleted.push(item);
            --i;
    ...
    value: function delEmptyLines(deleteedId) {
      for (var i = 0; i < this.data.pens.length; i++) {
        if (this.data.pens[i].type !== _pen.PenType.Line) {
          continue;
        }
    ...
    value: function cut() {
      ...
      this.clipboard = new _data.TopologyData({
        pens: []
      });
      ...
      try {
        for (_iterator15.s(); !(_step15 = _iterator15.n()).done;) {
          ...
          if (pen instanceof _line.Line) {
            pen.id = (0, _uuid.s8)();
            pen.from = new _point.Point(pen.from.x + 20, pen.from.y + 20, pen.from.direction, pen.from.anchorIndex, idMaps[pen.from.id]);
            pen.to = new _point.Point(pen.to.x + 20, pen.to.y + 20, pen.to.direction, pen.to.anchorIndex, idMaps[pen.to.id]);
            var controlPoints = [];
            ...
            try {
              for (_iterator16.s(); !(_step16 = _iterator16.n()).done;) {
                ...
                controlPoints.push(new _point.Point(pt.x + 20, pt.y + 20));
              }
            } catch (err) {
              _iterator16.e(err);
            } finally {
    ...
    value: function newId(node, idMaps) {
      var old = node.id;
      node.id = (0, _uuid.s8)(); // 解决图表组件复制粘贴只显示一个问题
      ...
      try {
        for (_iterator18.s(); !(_step18 = _iterator18.n()).done;) {
          ...
          if (pen instanceof _node.Node) {
            if (pen.autoRect) {
              var ctx = this.canvas.canvas.getContext('2d');
              var rect = (0, _middles.calcTextRect)(ctx, pen);
    ...
    value: function up(pen, pens) {
      ...
      if (i > -1 && i !== pens.length - 1) {
        ...
        var parent = (0, _canvas.getParent)(pens, pen);
    ...
    value: function getParent(pen) {
      return (0, _canvas.getParent)(this.data.pens, pen);
    }
  }, {
    key: "combine",
    value: function combine(pens) {
      ...
      var node = new _node.Node({
        name: 'combine',
        rect: new _rect2.Rect(rect.x, rect.y, rect.width, rect.height),
        text: '',
        paddingLeft: 0,
        paddingRight: 0,
        paddingTop: 0,
    ...
    value: function find(idOrTag, pens) {
      ...
      return (0, _canvas.find)(idOrTag, pens);
    }
  }, {
    key: "findIndex",
    ...
    value: function translate(x, y, process, noNotice) {
      ...
      if (this.options.isApp) {
        ...
        _storeUtils.Store.set(this.generateStoreKey('LT:updateLines'), this.data.pens);
    ...
    value: function fitView(viewPadding) {
      ...
      var padding = (0, _padding.formatPadding)(viewPadding || this.options.viewPadding); // 4. 获取图形尺寸
    ...
    value: function getViewCenter(viewPadding) {
      var padding = (0, _padding.formatPadding)(viewPadding || this.options.viewPadding);
    ...
    value: function toComponent(pens) {
      ...
      try {
        for (_iterator27.s(); !(_step27 = _iterator27.n()).done;) {
          ...
          if (item.type === _pen.PenType.Node && rect.width === item.rect.width && rect.height === item.rect.height) {
            node = item;
    ...
    value: function setColor(color) {
      ...
      _storeUtils.Store.set(this.generateStoreKey('LT:color'), color);

      this.options.font.color = color;

      _storeUtils.Store.set(this.generateStoreKey('LT:fontColor'), color);
    }
  }, {
    key: "setFontColor",
    ...
    value: function setIconColor(color) {
      _storeUtils.Store.set(this.generateStoreKey('LT:iconColor'), color);
    }
  }, {
    key: "pureData",
